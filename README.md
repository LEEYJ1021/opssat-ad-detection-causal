...
> **Summary.** **Alarm design for telemetry anomaly detection has a default assumption that this paper tests and rejects: that the primary decision signal should be built on signal *level*.** We tested that assumption on labeled anomaly onsets in three benchmarks spanning two industrial domains — OPS-SAT-AD (9 ESA spacecraft telemetry channels, 5 in analytical scope), SMAP/MSL (82 NASA spacecraft channels), and SMD (1,064 industrial server-machine channels) — each against same-channel placebo pivots. **In all three datasets, without exception, onset-relative change features (`diff`, `diff²`) discriminate true anomaly onsets from placebo more strongly than absolute `level` — and in two of the three datasets, this holds with no onset-detection model in the loop at all**, since SMAP/MSL and SMD ship ground-truth onset labels.
>
> Measured as onset-vs-placebo AUROC with **cluster-bootstrap confidence intervals** (the unit of resampling is the segment for OPS-SAT-AD, the channel for SMAP/MSL, and the machine for SMD, so that repeated or non-independent pairs do not inflate significance), the advantage is **Δ(diff − level) AUROC = +0.462 [0.409, 0.508]** in OPS-SAT-AD, **+0.122 [0.045, 0.196]** in SMAP/MSL, and **+0.042 [0.016, 0.066]** in SMD. Adding `level` to `diff`/`diff²` in an out-of-fold logistic model provides essentially **no incremental discrimination** (ΔAUROC between −0.001 and −0.007 across datasets; Tier-1 evidence in §11) — **`level` is not a complementary signal to `diff`/`diff²`, it is redundant with them.**
>
> **We did not stop at direction.** A normal-segment control decomposes `diff`-first onset ordering into an **operator-level baseline** (the effect of differencing on local disturbances) and an **anomaly-attributable increment**, and the increment is positive in every dataset — +40.1 percentage points (pp) in OPS-SAT-AD, +11.0 pp in SMAP/MSL, and +3.9 pp in SMD. We then went further and stress-tested this decomposition against a synthetic AR(1)+drift generator (§11.6): the generator reproduces the *direction* of the baseline gradient (Spearman ρ = 0.86 with autocorrelation, p < 0.001) but not its *magnitude* (simulated baselines top out at 39.7%, versus 44–73% observed) — so we report the decomposition as a **calibration tool with a demonstrated limit**, rather than overselling it as a validated mechanism.
>
> At a matched false-alarm rate, we also priced the operational payoff rather than leaving it as an abstract significance claim: in SMD, a `level OR diff OR diff²` fusion rule raises coverage at 5% FAR from 15.9% (level alone) to 17.2%, a **+1.2 pp [0.4, 2.2]** cluster-bootstrap gain; a single derivative feature alone does not reach significance at this FAR (§11.4). In OPS-SAT-AD, pooled `level` shows no positive discrimination (AUROC 0.438 [0.401, 0.477]) and is significantly reversed in one channel (CADC0873, AUROC 0.289, two-sided cluster p = 0.002); **we investigated this reversal directly (§11.5) rather than leaving it unexamined, and it is partially, but not fully, explained.** **The result of this study is therefore a cross-dataset, dependence-aware, operationally quantified design rule for alarm-feature selection** — not a universally superior detector, not a demonstrated causal mechanism, and not a general instrument-physics claim, each of which we explicitly scope out below.
...
**The answer this study provides.** **Across three benchmarks spanning spacecraft and industrial platforms — different sensor physics, different labeling protocols, different onset provenance — variance surges in the first and second differences carry more placebo-separating information at labeled anomaly onsets than level does, with no exception across the three datasets.** Cluster-bootstrap re-analysis (§11) confirms this direction on a resampling unit that respects each dataset's dependence structure (segment / channel / machine — the design most reviewers of this kind of claim would ask for), and quantifies the operational, false-alarm-matched value of adding derivative features as real and, where measured, smaller than the raw significance numbers alone would suggest — a size we report rather than round up.

**Seven contributions.**

1. **A cross-dataset design rule that survives without any onset-detection model in the loop.** `diff`/`diff²` variance features separate onsets from placebo more strongly than `level` in **3/3 datasets, with no exception**. In the two datasets with ground-truth onsets (SMAP/MSL, SMD), **the direction holds even when the entire BOCPD/Kalman-filter estimator used in OPS-SAT-AD is removed from the pipeline** (Table 12, Fig. 6) — this is the load-bearing result, and it does not depend on the OPS-SAT-AD-specific machinery elsewhere in this repository.
2. **A cluster-robust, dependence-aware quantification of that gap — replacing, not supplementing, pooled p-values that would otherwise overstate significance.** ΔAUROC(diff − level) = +0.462 [0.409, 0.508] (OPS-SAT-AD, segment clusters), +0.122 [0.045, 0.196] (SMAP/MSL, channel clusters), +0.042 [0.016, 0.066] (SMD, machine clusters) (Table R2, §11.3).
3. **A false-alarm-matched measurement of the alarm-design payoff — turning "diff separates onsets better" into a number an operator can act on.** At 5% FAR, a `level|diff|diff²` fusion rule raises SMD coverage by +1.2 pp [0.4, 2.2] over `level` alone; a single derivative feature alone does not clear a significant gain at this operating point (Table R3, §11.4).
4. **An operator-baseline decomposition, reusable by any future feature-precedence study, that we ourselves stress-tested against a synthetic generator and reported honestly when it fell short.** A normal-segment control separates the operator-level baseline of differencing from the anomaly-attributable increment: +40.1 pp (OPS-SAT-AD), +11.0 pp (SMAP/MSL), +3.9 pp (SMD) (Table 13, Fig. 7). The synthetic existence-proof check (Table R5, §11.6) reproduces this gradient's *direction* but not its *magnitude* — we report this as a limit on the tool's precision, not bury it.
5. **A sharp instrument-physics reading in OPS-SAT-AD, including the one place it does not fully close, disclosed rather than smoothed over.** `diff`/`diff²` are significant in 5/5 channels (all p_bonf < 2×10⁻⁴) and the ordering *reverses* in normal controls (level first in 55.8% of normal pairs versus 15.7% of anomalous). `level` shows no positive pooled discrimination (AUROC 0.438) and is **significantly reversed** in one channel (CADC0873); a sensitivity check against the pooled-standard-deviation definition of `level` narrows but does not eliminate this reversal (Table R4, §11.5) — the cause remains open, and we name it as such.
6. **The most extensive verification chain applied to a result of this kind in this literature.** Monte Carlo propagation of onset uncertainty (200 draws), train/test/bootstrap triple verification, a 16-model classifier cross-check, leave-one-out heterogeneity decomposition, and a from-scratch replica of the entire canonical-onset/placebo-pool pipeline that reproduces the saved analysis dataset to a maximum relative error of 1.8×10⁻¹⁵ (§11.2, §11.7).
7. **Audit-grade transparency, applied to the re-analysis itself, not just the original pipeline.** A contamination of the pre-onset baseline in an early SMAP/MSL scheme was caught, discarded and fixed (§10, `docs/dev-log/`). The robustness re-analysis in §11 applies the same standard to its own outputs: a duplicate-placebo-pool check for the 16-model benchmark independently re-derives the tabular dataset from raw time series and cross-checks it under de-duplication, multi-seed grouped cross-validation, and a row-level leakage positive control (§11.7), with the headline agreement statistics unchanged.

**What this changes for practice.**

- **Add a derivative-variance channel to the alarm feature set.** This is the primary, well-supported recommendation of the paper, confirmed across three domains and robust to cluster-dependence correction. **At a matched false-alarm rate the measured gain is single-digit percentage points of coverage (§11.4) — we report that number precisely instead of letting the underlying significance tests imply something larger.**
- **Calibrate against the operator baseline before claiming an early-warning gain.** The decomposition in §11.6 gives a reusable procedure for doing this, with an explicit, tested boundary on what it can and cannot support (a calibration tool, not a validated causal account).
- **Prioritize by instrument type, provisionally.** Float-noise magnetometer channels show the strongest derivative signature, but this pattern is aliased with a third confound — detector-selection retention, which ranges 28–100% across the five OPS-SAT-AD channels (§11.2) — so it is reported as a preliminary hypothesis, not a rule.
...
Which results depend on a model-estimated onset, and which do not, determines how each is weighted. **This is one of the strongest features of the design**: the load-bearing claim sits on the estimator-free evidence, not on the OPS-SAT-AD-specific detector.
...
**How the results are weighted.** The strong-form OPS-SAT-AD readings (Tier 4) are stated as conditional on the onset estimator and on OPS-SAT-AD's instrument physics. **The onset-independent evidence (SMAP/MSL, SMD) is what makes this a cross-domain result rather than a single-benchmark finding**: it supports the directional claim (Tier 1) and its cluster-robust quantification (§11.3) with no detector in the loop at all. That is why the directional claim, rather than the strict-specificity claim, is the load-bearing result.
...
| `diff`/`diff²` separate onsets from placebo more strongly than `level` (**direction, cluster-robust**) | Fig. 3a, Table 3; **ΔAUROC +0.462 [0.409,0.508] (Table R2)** | **Holds in 3/3 datasets, no exception**; ΔAUROC +0.122 [0.045,0.196] (SMAP/MSL), +0.042 [0.016,0.066] (SMD) (Table 12, Fig. 6, **Table R2**) |
...
**Reading this table.** **Row 2 is the load-bearing result — it holds in every dataset tested, with no exception, and is now supported by a dependence-aware statistic rather than a pooled p-value that a reviewer could challenge on independence grounds.** Row 5 gives the operational size of that result at a stated false-alarm rate. Row 4 is reported as a decomposition (operator baseline plus anomaly-attributable increment) with an explicit calibration-not-mechanism caveat. Row 3 discloses the one open anomaly (CADC0873) rather than reporting a uniform null. Rows 6 and 7 record where the OPS-SAT-AD instrument-physics reading is sharper than the cross-dataset reading and where it has been independently re-derived from scratch.
...
**Reading this table.** At a matched 5% FAR, SMD's single `diff` feature alone shows a coverage gain over `level` whose cluster-bootstrap CI includes zero (Δ = +0.78 pp [−0.38, 1.84]), so a single-feature claim is not supported at this sample size and operating point. The `level|diff|diff²` **fusion rule**, however, clears a significant gain at both FAR targets tested, and this is the number carried into §13's practical recommendation: **add a derivative channel to the alarm logic, and expect a real, precisely quantified (~1 percentage point) coverage improvement at typical false-alarm budgets** — the kind of number an alarm-design paper can actually cite, rather than a bare significance claim. OPS-SAT-AD's FAR-matched comparison requires the raw telemetry trace at scale and is listed as a roadmap item (§20).
...
**Conclusion.** **The generator does exactly what an existence-proof check should do: it reproduces the *qualitative* existence of an operator baseline and its *direction* of increase with autocorrelation, confirming the §12 argument is on the right track, while cleanly failing to reproduce the *magnitude* of the observed baseline gradient — which is precisely the boundary we report, rather than the boundary a less careful analysis would have missed.** So **Tier 3 of the evidence ladder (the baseline decomposition) is reported as a descriptive calibration tool rather than as a validated causal mechanism.** A generator search that adds heteroscedasticity, regime-switching, discretization and heavy-tailed innovations is listed in the roadmap as the direct next test.
...
**This is a feature-choice recommendation with a stated, measured operational size — not just a direction.**

- **Feature choice for thresholding — the primary, well-supported recommendation.** Across all three datasets, `diff`/`diff²` variance features separate labeled onsets from placebo more strongly than `level`, on both per-feature rank tests (§ Layer 3.7, § External Validation) and cluster-robust AUROC (§11.3). **Adding a derivative-variance channel to the alarm feature set, alongside level-based thresholds, is the recommendation this paper is built to support, and it is supported in every dataset tested.**
- **The measured operational size.** At a matched 5% false-alarm rate, a `level|diff|diff²` fusion rule raised SMD coverage by +1.2 pp [0.4, 2.2] over `level` alone (§11.4). **We report this figure precisely, as the number a practitioner should expect, rather than let the much larger significance-test gaps elsewhere in this README imply a bigger operational win than the data supports.** A single derivative feature alone did not clear significance at this operating point and sample size; fusion, not substitution, is the supported recommendation.
...
**Effect of the external results.** In SMAP/MSL and SMD, `diff` precedes `level` in anomalous and normal regimes alike (Fig. 7a), so the OPS-SAT-AD normal-control reversal is specific to its regime. The small positive anomalous-minus-normal increments (Table 13, Fig. 7b) are compatible with an anomaly-specific component on top of the baseline. **A claim tested against external data spanning two industrial domains, cluster-robust resampling, a false-alarm-matched operating point, and a synthetic existence-proof — and scoped accordingly, in each case, where the check came back partial or negative — is more transferable than one asserted from a single benchmark and a single statistic, and that is the comparison this paper is built to win.**
